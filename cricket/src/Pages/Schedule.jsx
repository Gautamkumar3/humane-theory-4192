import { Box, Button, CircularProgress, SimpleGrid, Stack, Text } from '@chakra-ui/react'
...
const getData = (filter) => {
    return axios.get(`https://gautam-todos-api.herokuapp.com/data?q=${filter}`)
}
...
const Schedule = () => {
    ...
    return (
        <Box w="70%" m="auto" p={5} bg="white">
            <Text fontWeight="bold" fontSize="lg" letterSpacing={1}>Schedule</Text>
            <Stack direction='row' spacing={4} align='center' my={5}>
                <Button onClick={() => setFilter("Match not started")} bg='#9b000d' variant='solid' color="white" w="20%">
                    UPCOMING
                </Button>
                <Button onClick={() => setFilter("won")} colorScheme='teal' variant='outline' w="20%">
                    RESULTS
                </Button>
